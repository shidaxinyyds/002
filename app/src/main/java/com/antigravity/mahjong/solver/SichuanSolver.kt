...
object SichuanSolver {
    ...
    data class AnalysisResult(
        val phase: String, // "DINGQUE_FORCED" | "READY_HAND_TING" | "SEARCHING_SHANTEN"
        val message: String,
        ...
        for (d in uniqueHand) {
            ...
            for (waitTile in 0..26) {
                ...
                if (canHu(testHand, numFixed)) {
                    waits[waitTile] = remainingCounts[waitTile]
                }
            }

            if (waits.isNotEmpty()) {
                val totalWins = waits.values.sum()
